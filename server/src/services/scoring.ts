import { Friend, ScoredPlace, PlaceType } from "../../../shared/types";
...
export function computeMidpoint(friends: Friend[]): { lat: number; lng: number } {
  if (friends.length === 0) return { lat: 0, lng: 0 };
  if (friends.length === 1) return { lat: friends[0].lat, lng: friends[0].lng };
...
function fairnessScore(distances: number[]): number {
...
export function scorePlaces(
  ...
  const scored: ScoredPlace[] = places.map((place) => {
    const distances = friends.map((f) =>
      haversine(f.lat, f.lng, place.lat, place.lng)
    );
    const avg = distances.reduce((a, b) => a + b, 0) / distances.length;
